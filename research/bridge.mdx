...
title: "Bridge"
sidebarTitle: "Bridge"
description: "L1 to Ethereum bridge mechanics. Deposit flow, withdrawal flow, validator set updates, and bridge state management."
icon: "bridge"
---

The HyperLiquid bridge handles all asset movement between the L1 and Ethereum mainnet. It is a **validator-voted system**: both deposits and withdrawals require quorum agreement from the active bridge validator set before execution.
...
Deposits move assets from Ethereum to HyperLiquid L1. The flow requires validator quorum before crediting the user.
...
Withdrawals move assets from HyperLiquid L1 back to Ethereum. The process includes a time-lock period and requires validator signatures.
...
<Steps>
  ...
  <Step title="Withdrawal complete">
    Once quorum is reached, the withdrawal is marked as `WithdrawalFinalized` and recorded in `finished_withdrawal_to_time`. Funds arrive on Ethereum.
  </Step>
</Steps>
