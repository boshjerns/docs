...
| `cloid` | string | Client order ID (if set) |
...
- Response size for BTC is ~15MB (33,000+ orders). Use compression (`Accept-Encoding: gzip`).
- Data refreshes every 1 second from our local node.
- Trigger orders that have not been activated are **not included**.
- Spot order books are **not included** in the current version.
