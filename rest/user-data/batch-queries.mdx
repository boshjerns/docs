...
<Note>
Native HyperLiquid info API only supports single-wallet queries. hypernode fans out to our local HL node with tuned concurrency control, returning results for up to 10,000 wallets per request in under 1.5 seconds.
</Note>
...
Per-wallet latency scales linearly because requests fan out in parallel against the local HL node with tuned concurrency control. Response times remain stable under load.
