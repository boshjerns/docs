...
|-------------|---------|
| `"order"` | `order_open`, `order_filled`, `order_canceled`, `order_rejected` |
| `"fill"` | `fill` |
| `"cancel"` | `cancel` |
| `"cancelByCloid"` | `cancelByCloid` |
| `"batchModify"` | `batchModify` |

You can also use exact type names like `"order_open"` to match a single event type.
...
{"action": "subscribe"}
```
~6,500 events per 5 seconds on mainnet. Includes both pre-consensus and confirmed events.
